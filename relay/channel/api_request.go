...
var passthroughSkipHeaderNamesLower = map[string]struct{}{
	...
	// 企业身份协议 Header 一律不得通过 wildcard/regex/{client_header} 透传给上游。
	// AIIdentityAuth 也会在入口删除它们；这里是纵深防护。
	...
	// W3C Trace Context（§9.16）：不得由 passthrough 原样转发，只能由 OTel Propagator
	// 基于当前 Child Span 注入。
	"traceparent": {},
	"tracestate":  {},
...
func processHeaderOverride(info *common.RelayInfo, c *gin.Context) (map[string]string, error) {
	...
	// 纵深防护：企业身份协议 Header（含显式 override 或 {client_header} 注入）一律
	// 不得进入上游请求头。
	for name := range headerOverride {
		if appconstant.IsAIAttributionHeader(name) {
...
func DoWssRequest(a Adaptor, c *gin.Context, info *common.RelayInfo, requestBody io.Reader) (*websocket.Conn, error) {
	...
	// 使用带 Context 的 Dial，并通过 Propagator 注入 W3C Trace Context（§9.17）。
	tracing.InjectTraceContext(c.Request.Context(), targetHeader)
